<?php
	...
	$accounts = array();
	$p = getPagination("SELECT a.id, a.email, concat(x.firstname, ' ', x.lastname) as name, x.address FROM account_user x  INNER JOIN account a ON a.id = x.account  WHERE a.validated = 0 ORDER BY x.firstname, x.lastname");
	$stmt = $p['stmt'];
	$stmt->execute();
